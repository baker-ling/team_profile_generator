...
function createTeamHTMLFile(team) {
  ...
  const OUTPUT_PATH = '';
  const TEAM_NAME_PLACEHOLDER = 'TEAM_NAME_PLACEHOLDER';
  ...
  fs.readFile(TEMPLATE_PATH, (err, data)=>{
    ...
    output.replace(TEAM_NAME_PLACEHOLDER, team.name);

    const employeeCardsHTML = team.employees.map(employee => generateCardHTML(employee)).join('\n');
    output.replace(EMPLOYEE_CARDS_PLACEHOLDER, employeeCardsHTML);
